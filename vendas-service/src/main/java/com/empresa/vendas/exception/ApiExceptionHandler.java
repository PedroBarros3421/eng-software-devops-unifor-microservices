...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleFeignNotFound(FeignException.NotFound ex, HttpServletRequest request) {
        return buildResponse(HttpStatus.NOT_FOUND, "Recurso dependente nao encontrado", request.getRequestURI());
    }

    @ExceptionHandler(FeignException.UnprocessableEntity.class)
    public ResponseEntity<Map<String, Object>> handleFeignBusiness(FeignException.UnprocessableEntity ex, HttpServletRequest request) {
        return buildResponse(HttpStatus.UNPROCESSABLE_ENTITY, "Regra de negocio violada em servico dependente", request.getRequestURI());
    }
